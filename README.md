...
In this project, we explore a machine-learning-based alternative. We simulate large numbers of phylogenetic trees (5000) under controlled piece-wise constant BDS processes with two intervals, extract sampling and branching times from the trees, and train a neural network regressor to predict the true generating parameters directly from these times.
...
- Learn a stable mapping from tree-derived features (times) to BDS parameters
...
- Piecewise-constant birth rates (only birth rate changes) with a single rate shift at time t₁
- Explicit sampling process (ψ)

Each tree is transformed into a fixed-length feature vector (X) that represent sampling and branching times (2000 features with padding), paired with its true generating parameters (y).
...
- Trees are converted into numerical summary statistics (e.g., tip counts, tree height, temporal structure)
...
- Semantically incorrect features (e.g., swapped before/after t₁ counts) may still yield low loss but lead to misleading biological interpretations
...
- log1p transformation of targets (TODO, which one are transformed?)
- stronger regularization (dropout, weight decay)
- smaller architecture + early stopping (TODO a short decription of the architecture, # layers, embeding sizes)
- better hyperparameter choices and larger batch size

The model performs particularly well on ψ, t₁, and λ₂ (relative errors < 20% safeguarded), while μ remains the most challenging parameter due to its highly skewed distribution with many near-zero values.
...
- Use tree-aware architectures (e.g., graph neural networks)
...
TODO: a short overview of the repo structure
